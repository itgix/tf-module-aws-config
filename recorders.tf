...
resource "aws_config_configuration_recorder" "security_acc" {
...
resource "aws_config_configuration_recorder_status" "security_acc" {
  count      = var.is_security_account ? 1 : 0
  name       = local.recorder_name
  is_enabled = true

  depends_on = [aws_config_delivery_channel.security_acc]
}
...
resource "aws_config_delivery_channel" "security_acc" {
  ...
  depends_on = [aws_config_configuration_recorder.security_acc]
}
